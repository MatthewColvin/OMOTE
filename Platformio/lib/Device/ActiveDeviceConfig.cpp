...
ActiveDeviceConfig::ActiveDeviceConfig(std::shared_ptr<LittleFsInterface> fs)
    : m_fs(fs) {}
...
bool ActiveDeviceConfig::saveDevices(const std::vector<IDevice::Ptr> &devices) {
  ...
  auto file = m_fs->open(ACTIVE_DEVICES_CONFIG_FILE, LFS_O_WRONLY | LFS_O_CREAT);
  if (!file)
    return false;
...
std::vector<IDevice::Ptr> ActiveDeviceConfig::loadDevices() {
  ...
  auto file = m_fs->open(ACTIVE_DEVICES_CONFIG_FILE, LFS_O_RDONLY);
  if (!file)
    return devices;
  ...
  for (const auto &deviceObj : doc.GetArray()) {
    ...
    if (type == DeviceType::CompileTime) {
      device = DeviceFactory::Create(id);
    }
    if (deviceObj.HasMember("config")) {
      ...
      if (!device && deviceObj.HasMember("entityId") && deviceObj["entityId"].IsString()) {
        if (std::string entityIdFromConfig = deviceObj["entityId"].GetString(); !entityIdFromConfig.empty()) {
          // TODO encapsulate HA into the Factory.
          // device = DeviceFactory::CreateHomeAssistDevice(entityIdFromConfig);
        }
      }
