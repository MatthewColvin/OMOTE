...
bool Touch_FT5x26::_write_reg(uint8_t reg, uint8_t val) {
...
bool Touch_FT5x26::_read_reg(uint8_t reg, uint8_t *data, size_t length) {
  return lgfx::i2c::transactionWriteRead(_cfg.i2c_port, _cfg.i2c_addr, &reg, 1, data, length, _cfg.freq).has_value();
}

bool Touch_FT5x26::_check_init(void) {
  if (_inited)
    return true;

  uint8_t tmp[6] = {0};
  _inited = _write_reg(0x00, 0x00) && _read_reg(FT5x06_CIPHER_REG, tmp, 6) && _write_reg(FT5x06_INTMODE_REG, 0x00) // INT Polling mode
            && tmp[5];
#if defined(ESP_LOGV)
  if (_inited) {
    ESP_LOGV("FT5x46", "CIPHER:0x%02x / FIRMID:0x%02x / VENDID:0x%02x", tmp[0], tmp[3], tmp[5]);
  }
#endif
  return _inited;
}

bool Touch_FT5x26::init(void) {
  _inited = false;
...
void Touch_FT5x26::sleep(void) {
  if (!_check_init())
    return;
  _write_reg(FT5x06_POWER_REG, FT5x06_SLEEP_IN);
}

size_t Touch_FT5x26::_read_data(uint8_t *readdata) {
  /// 戻り値res: 通信に失敗した場合は0、通信に成功した場合はByte数
  size_t res = 0;
  if (lgfx::i2c::beginTransaction(_cfg.i2c_port, _cfg.i2c_addr, _cfg.freq, false)) {
    readdata[0] = 0x02;
    if (lgfx::i2c::writeBytes(_cfg.i2c_port, readdata, 1) && lgfx::i2c::restart(_cfg.i2c_port, _cfg.i2c_addr, _cfg.freq, true) && lgfx::i2c::readBytes(_cfg.i2c_port, readdata, 1)) {
      uint_fast8_t points = std::min<uint_fast8_t>(max_touch_points, readdata[0] & 0xFFu);
      if ((points > 0) && (points < 10)) // on power up seems to return 10 untill first touch??
      {
...
uint_fast8_t Touch_FT5x26::getTouchRaw(touch_point_t *tp, uint_fast8_t count) {
  if (!_check_init() || count == 0)
    return 0;
  if (_cfg.pin_int >= 0) {
  ...
  uint8_t readdata[2][62];
  size_t readlen[2];
  if (1 == (readlen[0] = _read_data(readdata[0]))) {
    return 0;
  }
  size_t comparelen;
  int32_t retry = 5;
  do { // 読出し中に値が変わる事があるので、連続読出しして前回と同値でなければリトライする;
