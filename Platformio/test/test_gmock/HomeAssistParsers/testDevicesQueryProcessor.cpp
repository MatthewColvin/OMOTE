...
class DevicesQueryProcessorTest : public Test {
 protected:
  ...
  void SetUp() override {
    ...
    messageProcessor =
        std::make_unique<MockIProcessMessage>(std::move(process));
  }
};
...
TEST_F(DevicesQueryProcessorTest, ShouldPreserveMemory) {
  auto testMaxMemUsage = 180;
  messageProcessor->SetMaxProcessBufferSize(testMaxMemUsage);

  std::vector<std::string> chunks = {
      R"({"id":37,"type":"result","success":true,"result":{"entity_categories":{"0":"config","1":"diagnostic"},"entities":[)",
      R"({"ei":"light.lamp","pl":"cync_lights","lb":[],"di":"4041e3f3eed9eea27e2a21b60484a27f","en":"Lamp"},)",
      R"({"ei":"light.shower","pl":"tuya","lb":[],"di":"22b11a1d3cfbcb9eb15d1f658de29106","hn":true},)",
      R"({"ei":"light.closet","pl":"tuya","lb":[],"di":"8313147ad6262f1162c1580de20d4bed","hn":true})",
      R"(]}})"};
  ...
  for (auto chunkIt = chunks.begin(); chunkIt < chunks.end() - 1; chunkIt++) {
    const bool result = messageProcessor->ProcessChunk(*chunkIt, totalLength);
    ASSERT_TRUE(result);
    ASSERT_EQ(messageProcessor->GetUnProcessedBufferCapacity(),
              testMaxMemUsage);
  }

  const bool result =
      messageProcessor->ProcessChunk(chunks.back(), totalLength);

  ASSERT_EQ(messageProcessor->GetUnProcessedBufferCapacity(), 0);
