...
## Desktop simulator (x64_sim)



Build and run from `Platformio` (see `Wiki/BasicUI.md`). In the editor **Connect** tab, use:
...
You must include **`http://`**. If you enter only `127.0.0.1:9080`, the browser treats it as a path on the editor server and you will see **Failed to fetch**.



That is the same JSON/IR API as hardware (`http://omote.local` on port 80). Files are read/written under `Platformio/sim_data/`. **Save to remote** does not exit the sim — it hot-reloads config in the running window (unlike hardware, which reboots).



Run the editor on a **different** port so it does not clash with the sim, e.g. `python -m http.server 8765` → open http://localhost:8765.



Optional sim-only WiFi setup page (not the editor API): **http://127.0.0.1:8080** — enter any SSID and password (plain text, not a URL).



| Target | Editor Connect URL |

|--------|-------------------|

| Hardware on LAN | `http://omote.local` or `http://<your-mdns-name>.local` or `http://<device-ip>` |

| Simulator | `http://127.0.0.1:9080` |
